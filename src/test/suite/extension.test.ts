...
suite('Extension entry point test:', function() {
	const extensionId = 'redhat.vscode-camelk';

	suite('test extension availability', function() {
	...
	suite('test getBaseCmd', function() {
	...
	suite('test settings', function() {
		...
		let initialRuntimeVersion :string | undefined;
		let initialNamespace :string | undefined;
		let initialAutoUpgrade :boolean;
		
		this.beforeAll(async() => {
			initialRuntimeVersion = config.getKamelRuntimeVersionConfig();
			initialNamespace = config.getNamespaceconfig();
			initialAutoUpgrade = config.getKamelAutoupgradeConfig();
		});
			
		this.afterAll(async() => {
			await config.setKamelRuntimeVersionConfig(initialRuntimeVersion as string);
			await config.addNamespaceToConfig(initialNamespace);
			await config.setKamelAutoupgradeConfig(initialAutoUpgrade);
		});
